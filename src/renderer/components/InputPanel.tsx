...
export default function InputPanel({ entriesList, setEntriesList }) {
  ...
  useEffect(() => {
    ...
    if (timerStatus === 'initial') setCurrentSeconds(0);
  ...
  useEffect(() => {
    ...
    if (submittingTask) {
      ...
      const diffrence = timerEndTime.diff(timerStartTime, 's');
      console.log('====================================');
      ...
      if (isEntryValid(taskDescription, taskSecondsCount)) {
        const newEntry = {
          ...
          timerStartTime: timerStartTime.format('HH:mm:ss'),
          timerEndTime: timerEndTime.format('HH:mm:ss'),
        };
