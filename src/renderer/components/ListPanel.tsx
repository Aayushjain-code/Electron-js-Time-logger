...
export default function ListPanel({
  ...
  const [editableEntries, setEditableEntries] = useState(
    entriesList.map((_) => false)
  );

  useEffect(
    () => setEditableEntries(entriesList.map(() => false)),
    [entriesList]
  );

  const handlePressedEdit = (id, taskIndex) => {
    console.log(
      `Requested to edit the task with id ${id} in the position ${taskIndex}`
    );
    setEditableEntries(
      editableEntries.map((value, index) =>
        index === taskIndex ? true : value
      )
    );
  };
